...
export class RoomsComponent extends AppComponentBase implements OnInit {
  ...
  createRoom() {
    const modal: BsModalRef = this._modalService.show(CreateRoomComponent, { class: 'modal-lg' });

    // ✅ Safe subscription
    if (modal.content && modal.content.onSave) {
      modal.content.onSave.subscribe(() => this.loadRoom());
    }
  }
